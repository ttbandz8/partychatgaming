    ...
    async def marry(self, ctx, player: User):
        try:
            ...
            if head_profile['LEVEL'] < 11:
                await ctx.send(f"🔓 {ctx.author.mention} Unlock Family by completing Floor 10 of the 🌑 Abyss! Use /abyss to enter the abyss.")
                return
            if partner_profile['LEVEL'] < 11:
                await ctx.send(f"🔓 {player.mention} Unlock Family by completing Floor 10 of the 🌑 Abyss! Use /abyss to enter the abyss.")
                return
