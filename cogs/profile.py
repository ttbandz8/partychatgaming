...
class Profile(commands.Cog):
    ...
    async def menu(self, ctx, selection):
        if selection == "cards":
            await cards(self, ctx)
        if selection == "titles":
            await titles(self, ctx)
        if selection == "arms":
            await arms(self, ctx)
        if selection == "build":
            await build(self, ctx)
        if selection == "summons":
            await summons(self, ctx)
        if selection == "storage":
            await storage(self, ctx)
        if selection == "destinies":
            await destinies(self, ctx)
        if selection == "quests":
            await quests(self, ctx)
        if selection == "balance":
            await balance(self, ctx)
        if selection == "presets":
            await preset(self, ctx)
        if selection == "savepreset":
            await savepreset(self, ctx)
        if selection == "shop":
            await shop(self, ctx)
        if selection == "craft":
            await craft(self, ctx)
        if selection == "gems":
            await gems(self, ctx)
...
async def trinketshop(self, ctx):
   user_query = {'DID': str(ctx.author.id)}
